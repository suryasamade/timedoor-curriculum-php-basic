...
<?php
    ...
    $querySelectPersons = $dbh->query("SELECT persons.id AS id, name, age, gender, height, weight, waist_circumference, score_rfm, rfm.category AS category_rfm, score_bmi, bmi.category AS category_bmi FROM persons INNER JOIN rfm ON persons.id_rfm=rfm.id INNER JOIN bmi ON persons.id_bmi=bmi.id");
...
<h3>Body Mass Index (BMI) and Relative Fat Mass (RFM) Category Calculator</h3>
    <p>Form input</p>
    <form action="c3t5_practice_create.php" method="POST">
    ...
    <table border="1">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href=<?="c3t5_practice_edit.php?id={$id}"?>>Edit</a> | <a href=<?="c3t5_practice_delete.php?id={$id}"?>>Delete</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
