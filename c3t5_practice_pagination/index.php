...
<?php
    ...
    $totalRecord = count($database->selectAll("persons"));
    $recordsPerPage = 2;
    $currentPage = (int) get_input('page', 1, 'get');
    
    $pagination = new Pagination($totalRecord, $recordsPerPage, $currentPage);
    $offset = $pagination->getOffset();
    
    $personsData = $database->selectLimit("persons", $offset, $recordsPerPage);
...
<h3>Body Mass Index (BMI) and Relative Fat Mass (RFM) Category Calculator</h3>
    ...
    <?php foreach ($pagination->getPages() as $page): ?>
        <a href="?page=<?= $page ?>" <?= $pagination->getCurrentPage() == $page ? "style='font-weight: bold; color: blue;'" : "" ?> ><?= $page ?></a>
    <?php endforeach ?>
</body>
